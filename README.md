# Роскомнадзор на GitHub

> *Мы пришли проверить ваши репозитории. И ваши мемы тоже.*

[![GitHub Pages](https://img.shields.io/badge/GitHub%20Pages-Live-brightgreen?logo=github)](https://yourusername.github.io/roskomnadzor-github)
[![Made with](https://img.shields.io/badge/Made%20with-Material%20Design-blue?logo=material-design)](https://material.io)
[![License](https://img.shields.io/badge/License-AGPL--v3.0-red)](LICENSE)
[![Status](https://img.shields.io/badge/Status-%D0%97%D0%B0%D0%B1%D0%BB%D0%BE%D0%BA%D0%B8%D1%80%D0%BE%D0%B2%D0%B0%D0%BD%D0%BE-critical)](https://rkn.gov.ru)

<img width="1902" height="1079" alt="image" src="https://github.com/user-attachments/assets/7a86b968-bdab-468a-9045-51f340b9b5b3" />


Шуточный лендинг в стиле Material Design, посвящённый легендарной Федеральной службе по надзору в сфере связи, информационных технологий и массовых коммуникаций — теперь и на GitHub!

## Фичи

- **Интерактивный сканер репозиториев** — проверь любой репозиторий на соответствие законодательству

<img width="1899" height="1079" alt="image" src="https://github.com/user-attachments/assets/801b4e49-3fc9-4712-b224-93d25489c3d9" />


- **6 официальных правил** для разработчиков (README на русском обязателен!)
- **Реестр заблокированных репозиториев** — с бейджами и причинами
- **FAQ** с ответами на «серьёзные» вопросы
- Material Design с анимациями, тенями и ripple-эффектами
- Полностью адаптивный

## Деплой

Просто открой `index.html` в браузере или залей на GitHub Pages:
```bash
git clone https://github.com/roskommnadzor/RKN.git


